...
### 1. Source install / update (Go >= 1.22)
...
```bash
cd /path/to/hiagent-go-sdk
git pull
cd cmd/hibot
go install .
```

This avoids GitHub release-asset downloads and the GitHub Releases API. Do not
use `go install github.com/volcengine/hiagent-go-sdk/cmd/hibot@latest` until a
CLI release tag is published; the development module uses local workspace
wiring for the sibling SDK module.
